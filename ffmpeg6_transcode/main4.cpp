...
const char* inFilePath = "http://127.0.0.1/test/out/hls.m3u8";
//const char *inFilePath  = "rtmp://192.168.3.202:1935/live/test";
const char* outFilePath = "E:/clib/data/hls-pull-test.flv";
...
void Step1_OpenInFile() {
    //STEP::打开源视频文件
    ret = avformat_open_input(&inFileHandle, inFilePath, NULL, NULL);
    if (ret < 0) {
...
void Step3_Operation() {
    ...
    while (av_read_frame(inFileHandle, packet) >= 0) {
        ...
        //std::cout << "av_read_frame" << stopTag << std::endl;
        //stopTag--;
        //if (stopTag==0) {
        //    break;
        //}
