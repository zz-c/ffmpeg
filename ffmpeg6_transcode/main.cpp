...
const char* outFilePath = "E:/clib/data/test-cg-transH265.mp4";
...
void Step_OpenDecoder() {
    ...
    for (int i = 0; i < streamContextLength; i++) {
        ...
        decoder->time_base = r;//AV_TIME_BASE_Q;                                                    //固定TimeBase为1/1000000，能防止能多奇怪问题，Fixed TimeBase is 1/1000000，can prevent strange problems
        ...
        decoder->time_base = r;                                                    //一些编码器会修改timebase，这里做一次覆盖设置，Some encoders modify timebase, do an override setting here
...
void Step_OpenEncoder() {
    ...
    for (int i = 0; i < streamContextLength; i++) {
        ...
        AVRational r = { 1, 1000000 };
        encoder->time_base = r;                                                    //固定TimeBase为1/1000000，能防止能多奇怪问题，Fixed TimeBase is 1/1000000，can prevent strange problems
        ...
        encoder->time_base = r;                                                    //一些编码器会修改timebase，这里做一次覆盖设置，Some encoders modify timebase, do an override setting here
